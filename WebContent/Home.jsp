...
<c:forEach var="temp" items="${allPosts}">
	<div class="postDiv">
	<table>
	<tr><td>${allUsers.get(temp.getUserId())}</td></tr>
	<tr>
	<c:choose>
	...
	<td><a href="./PostActions?act=del&pId=${temp.getPostId()}&page=home"><button>Delete</button></a></td>
	</c:when>
	<c:otherwise>
	<tr><td  colspan = "3"><textarea disabled>${temp.content}</textarea></td></tr>
	<td><a href="./PostActions?act=sav&pId=${temp.getPostId()}&page=home"><button>Save</button></a></td>
	</c:otherwise>
	</c:choose>
	<tr><td><a href="./PostActions?act=lik&pId=${temp.getPostId()}&page=home"><button>Like</button></a><input type="text" placeholder="${temp.getLikes()}" disabled id="lik"/></td>
